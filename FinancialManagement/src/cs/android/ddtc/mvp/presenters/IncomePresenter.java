...
public class IncomePresenter {
	...
	private final IIncomeView myView;
	...
	public IncomePresenter(IIncomeView view, JSONParser model) {
		myView = view;
		myModel = model;
	...
	public void onUpdateClick(String uid, String start, String end) throws ParseException {
		...
		new UpdateIncome(uid, start, end).execute();
	}
	
	class UpdateIncome extends AsyncTask<String, String, String>{
		String ID, START, END;
		double totalsalary, totalsavings, totalmisc, total;
		
		public UpdateIncome(String uid, String start, String end){
			ID = uid;
			START = start;
			END = end;
		}
		...
		protected void onPreExecute() {
			...
			pDialog.setMessage("Calculating income. Please wait...");
		...
		protected String doInBackground(String... arg0) {
			...
			params.add(new BasicNameValuePair("CATEGORY", "Savings"));
			
			JSONObject json = myModel.makeHttpRequest(Constants.url_translist, "GET", params);
				...
				if(success == 1){
					...
					for(int i = 0; i < totals.length(); i++){
						...
						if (timeDateVar.after(startDateVar)
							...
							totalsavings += c.getDouble(Constants.TAG_AMOUNT);
						}
			...
			params1.add(new BasicNameValuePair("CATEGORY", "Salary"));
			
			JSONObject json1 = myModel.makeHttpRequest(Constants.url_translist, "GET", params1);
			
			try{
				int success = json1.getInt(Constants.TAG_SUCCESS);
				if(success == 1){
					...
					for(int i = 0; i < totals.length(); i++){
						...
						if (timeDateVar.after(startDateVar)
								&& timeDateVar.before(endDateVar)) {
							totalsalary += c.getDouble(Constants.TAG_AMOUNT);
						}
			...
			params2.add(new BasicNameValuePair("CATEGORY", "MISC"));
			
			JSONObject json2 = myModel.makeHttpRequest(Constants.url_translist, "GET", params);
			...
			try{
				...
				if(success == 1){
					...
					for(int i = 0; i < totals.length(); i++){
						...
						if (timeDateVar.after(startDateVar)
								&& timeDateVar.before(endDateVar)) {
							totalmisc += c.getDouble(Constants.TAG_AMOUNT);
						}
		...
		protected void onPostExecute(String file_url) {
			pDialog.dismiss();
			total = totalsavings + totalsalary + totalmisc;
			myView.advanceToUpdate(totalsavings, totalsalary, totalmisc, total);
		}
	...
	class UpdateGraph extends AsyncTask<String, String, String>{
		...
		protected void onPostExecute(String file_url) {
			pDialog.dismiss();
			total = totalsavings + totalsalary + totalmisc;
			myView.advanceToGraph(totalsavings, totalsalary, totalmisc, total);
		}
